...
// 获取/释放 临时缓冲区
// `len` 表示需要申请的内存大小，`T*` 表示指向类型 `T` 的指针
template <class T>
pair<T*, ptrdiff_t> get_buffer_helper(ptrdiff_t len, T*) {
    // 检查 `len` 是否超过了最大可分配的内存大小，如果超过了，则将 `len`
    // 限制为最大可分配的大小。
    ...
    while (len > 0) {
        // 循环申请内存，直到申请成功或者 `len` 为 0。
        // 该函数会使用 `malloc` 函数申请 `len` 个 `T` 类型的空间，
        T* tmp = static_cast<T*>(malloc(static_cast<size_t> (len) * sizeof(T)));
        if (tmp) {
            // 如果申请成功，则返回一个指向该空间的指针和 `len`，表示成功申请了
            // `len` 个 `T` 类型的空间
            return pair<T*, ptrdiff_t>(tmp, len);
        }
        // 如果申请失败，则将 `len` 减半，继续尝试申请，直到申请成功或者 `len`
        // 为 0，最终返回一个空指针和 0，表示申请失败。
        len /= 2;  // 申请失败时减少len的大小
    }
...
public:
    ...
    ptrdiff_t request_size() const noexcept {
        return original_len;
    }
    T* begin() noexcept {
        return buffer;
