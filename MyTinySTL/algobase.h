...
/* 这是一个 C++ 中的模板函数，名为 `unchecked_copy`。它的作用是将指针 `first` 和
`last` 所指向的一段内存中的数据拷贝到指针 `result`
所指向的内存中。这个函数会返回一个指针，指向已经完成拷贝的最后一个元素的下一个位置。

这个函数模板有两个模板参数 `Tp` 和
`Up`，分别表示输入迭代器所指向的元素类型和输出迭代器所指向的元素类型。这个函数使用了
`std::enable_if` 模板和
`std::is_same`、`std::remove_const`、`std::is_trivially_copy_assignable`
等类型特性工具来进行类型检查和条件编译，在满足一定条件的情况下才会编译通过。

具体来说，这个函数模板的条件编译要求 `Tp` 和 `Up` 的类型相同，并且 `Up`
类型是可平凡复制赋值（trivially copy
assignable）的类型。如果满足这些条件，那么这个函数模板就会被实例化，并且返回
`result+n`，其中 `n`
是拷贝的元素个数。如果不满足这些条件，那么这个函数模板就不会被实例化。 */
