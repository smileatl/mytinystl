...
private:
    ...
    // sort
    template <class Compared>
    iterator list_sort(iterator first,
                       iterator last,
                       size_type n,
                       Compared comp);
};
...
typename list<T>::iterator list<T>::erase(const_iterator pos) {
    ...
    // 获取要删除节点的指针 n，以及它的下一个节点指针 next
    auto n = pos.node_;
    auto next = n->next;
    // 用 unlink_nodes 函数将节点 n 从链表中删除
    unlink_nodes(n, n);
    // 调用 destroy_node 函数销毁节点 n
    destroy_node(n->as_node());
    --size_;
...
void list<T>::splice(const_iterator pos, list& x) {
    ...
    MYSTL_DEBUG(this != &x);
    if (!x.empty()) {
        // 当前 list 的大小加上参数中的 list 的大小是否超过了 max_size
...
// 接受一个一元谓词pred，用于判断列表中的元素是否需要被一处
template <class T>
template <class UnaryPredicate>
...
// 可以用于去除链表中相邻的重复元素
// 接受一个二元谓词 pred，用于判断两个元素是否相等。
// 函数会遍历链表，如果相邻的两个元素相等，则删除后面的元素，直到链表中不存在相邻的重复元素为止
template <class T>
template <class BinaryPredicate>
void list<T>::unique(BinaryPredicate pred) {
    ...
    while (j != e) {
        if (pred(*i, *j)) {
            // 如果 pred(*i, *j) 返回 true，则删除 j 指向的元素
            erase(j);
        } else {
            // 否则将 i 移动到 j 的位置，继续比较下一对相邻元素。循环直到 j 指向链表的末尾
            i = j;
        }
        j = i;
        ++j;
